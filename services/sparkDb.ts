...
export const sparkDb = {
	...
	pollEvents(): SparkEvent[] {
		...
		db.transaction(() => {
			const rows = selectUnconsumedStmt().all() as {
				id: number;
				data: string;
			}[];
	...
	getRecentEvents(limit: number = 50): SparkEvent[] {
		const rows = getDb().prepare(`SELECT data FROM events ORDER BY id DESC LIMIT ?`).all(limit) as {
			data: string;
		}[];
		const events: SparkEvent[] = [];
		for (const row of rows) {
	...
	cleanup(): void {
		...
		db.prepare(`DELETE FROM events WHERE consumed = 1 AND timestamp < $cutoff`).run({
			$cutoff: cutoff,
		});
		db.prepare(`DELETE FROM access_logs WHERE timestamp < $cutoff`).run({
			$cutoff: cutoff,
		});
		...
		try {
			...
			if (sizeMB > maxSizeMB) {
				...
				db.prepare(`DELETE FROM events WHERE consumed = 1 AND timestamp < $cutoff`).run({
					$cutoff: aggressiveEventCutoff,
				});
				db.prepare(`DELETE FROM access_logs WHERE timestamp < $cutoff`).run({
					$cutoff: aggressiveAccessCutoff,
				});
				db.exec("VACUUM");
			}
		} catch {}
