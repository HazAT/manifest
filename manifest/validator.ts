...
function validateString(field: StringFieldDef, value: unknown): string | null {
	...
	if (field.format) {
		switch (field.format) {
			...
			case "date":
			case "datetime":
				if (Number.isNaN(Date.parse(value))) return "invalid_format";
				break;
		}
	}
	if (field.pattern) {
...
export function validateInput(
	...
	for (const [name, field] of Object.entries(schema)) {
		...
		switch (field.type) {
			case "string":
				error = validateString(field, value);
				break;
			case "integer":
				error = validateInteger(field, value);
				break;
			case "number":
			...
			case "boolean":
				...
				break;
			case "array":
				error = validateArray(field, value);
				break;
		}
