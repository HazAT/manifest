	...
	const imports = tsFiles.map(async (file) => {
		...
		try {
			...
			if (feature && typeof feature === "object" && feature.name) {
				(feature as unknown as Record<string, string>)._sourcePath = path.relative(
					process.cwd(),
					fullPath,
				);
...
export async function scanAllFeatures(projectDir: string): Promise<FeatureRegistry> {
	...
	for (const [name, feature] of Object.entries(projectRegistry)) {
		if (registry[name]) {
			console.warn(
				`[manifest] Feature '${name}' in project overrides extension feature at ${(registry[name] as unknown as Record<string, string>)._sourcePath}`,
			);
		}
