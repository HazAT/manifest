...
import type { StreamFeatureDef } from "./feature";
...
export function createTestClient(options: { featuresDir: string }): TestClient {
	...
	return {
		async call(featureName: string, input: Record<string, unknown>): Promise<TestResult> {
			...
			if (feature.type === "stream") {
				throw new Error(
					`Feature '${featureName}' is a stream feature. Use client.stream() instead of client.call().`,
				);
			}
		...
		async stream(featureName: string, input: Record<string, unknown>): Promise<StreamEvent[]> {
			...
			if (feature.type !== "stream") {
				throw new Error(
					`Feature "${featureName}" is not a stream feature (type: "${feature.type || "request"}")`,
				);
			}
