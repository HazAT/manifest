...
export default function devServer(pi: ExtensionAPI) {
		...
		switch (state) {
			case "running":
				statusCtx.ui.setStatus(
					"dev-server",
					theme.fg("success", `🟢 Server http://localhost:${port}`),
				);
				break;
			case "stopped":
	...
	async function startServer(): Promise<{ ok: boolean; message: string }> {
		...
		if (!freePort) {
			return {
				ok: false,
				message: `Port ${port} is occupied and no free port found in range ${port + 1}-${port + MAX_PORT_SCAN}.`,
			};
		}
	...
	pi.registerCommand("server", {
		...
		handler: async (args, ctx) => {
			...
			if (!(await handleServerCommand(sub))) {
				ctx.ui.notify(
					`Unknown subcommand: ${sub}. Use start, stop, restart, or status.`,
					"warning",
				);
			}
		},
