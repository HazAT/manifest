...
import { auth, InvalidCredentialsError, buildSessionCookie } from '../services/auth'
...
export default defineFeature({
  ...
  authentication: 'none',
  sideEffects: [
    'Creates one session row in sessions table',
  ...
  async handle({ input, ok, fail }) {
    try {
      const { user, sessionToken } = await auth.login(
        String(input.email),
