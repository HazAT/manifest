...
export default function spark(pi: ExtensionAPI) {
	...
	function getEnvProfile(cfg: SparkConfig) {
		return (
			cfg.environments[cfg.environment] ||
			cfg.environments.development || { tools: "full", behavior: "fix" }
		);
	}
	...
	function formatEvent(event: SparkEvent): string {
		...
		if (event.type === "process-error") {
			...
			lines.push(
				`**rate-limit**${event.feature ? ` on \`${event.feature}\`` : ""}${event.route ? ` — ${event.route}` : ""}`,
			);
			...
			lines.push(
				`**${event.type}**${event.feature ? ` in \`${event.feature}\`` : ""}${event.route ? ` — ${event.route}` : ""}`,
			);
	...
	async function readConfig(cwd: string): Promise<SparkConfig | undefined> {
		...
		try {
			...
			return {
				...
				environment:
					raw.environment || process.env.SPARK_ENV || process.env.NODE_ENV || "development",
				db: {
			...
			try {
				...
				return {
					...
					db: {
						...
						cleanup: {
							intervalMs:
								Number(raw.match(/intervalMs\s*:\s*([\d_]+)/)?.[1]?.replace(/_/g, "")) || 300_000,
						},
					},
					watch: {
...
`;
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		try {
			...
			report.push(
				`Git: branch \`${branch}\`${uncommitted > 0 ? `, ${uncommitted} uncommitted file${uncommitted > 1 ? "s" : ""}` : ", clean"}`,
			);
		} catch {
		...
		// App status
		try {
			const manifestConfig = await import(
				`${path.join(ctx.cwd, "config", "manifest.ts")}?t=${Date.now()}`
			);
